...
v1.1 统一约定:策略唯一允许 import 的平台包是 `core.contract`。`core.contract` 内只放稳定契约,不放任何具体数据源、回测器、实盘网关、风控实现。

## 1. 基础数据结构

```python
# core/contract/types.py
...
说明:契约类型面向策略和引擎交互,不是券商原始字段全集。券商特有字段保存在 OMS 的扩展表和事件流水中;只有跨回测、paper、实盘都稳定有意义的字段才进入 `core.contract`。

## 2. 策略生命周期

```python
# core/contract/strategy.py
...
- 风控拒单表现为 `on_order` 收到 `REJECTED` 状态,`Order.reject_reason` 与日志/事件流水同时记录原因——策略应当能优雅处理被拒。
- `set_target` 与 `order` 不要混用同一标的,避免执行模块和策略互相打架。
- 运行模式(`backtest/paper/live`)不暴露给策略逻辑。模式写入配置快照、日志和报告,但策略不能通过 Context 读取后分支。
...
5. 禁止探测运行模式并参与逻辑分支。`Context` 不提供 `mode`;策略也不得通过配置、环境变量、文件路径等方式绕过。
...
runtime_mode: backtest          # backtest | paper | live,由启动器/引擎读取,不传入 ctx.params
```
...
from core.contract.strategy import StrategyBase
from core.contract.types import Bar
