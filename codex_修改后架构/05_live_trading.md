...
换市场 = 换一个 Gateway 实现 + 换合约表,核心层与策略零改动。Gateway 返回的是券商侧快照;OMS 负责把它规范化为 `core.contract.Order/Trade` 并维护本地 ID 映射。`BrokerOrderSnapshot` 是网关适配层 DTO,字段可随券商而异,不能直接暴露给策略。
...
- 本地 `order_id` 在发送**前**生成并落库(SQLite `orders` 表),同时记录 `strategy_id/account_id/symbol/side/qty/price/created_at`。收到 `broker_order_id` 后回填映射。
- 任何状态变更先写库、再派发回调(write-ahead),崩溃后状态可恢复。
- `reject_reason` 和券商错误码必须落库并写事件流水;风控拒单和券商拒单都统一表现为 `OrderStatus.REJECTED`。
...
v1.1 补充:OMS 是实盘事实整理者,不是策略工具箱。策略只能通过 `Context.order/set_target/cancel` 表达意图;任何券商原始状态、错误码、重连细节都由 OMS 转成契约事件后再回调策略。
...
`SimGateway` 实现同一个 GatewayBase 接口:订阅**真实实时行情**,撮合在本地模拟(复用回测的 Matcher 和成本模型)。配置里 `runtime_mode: paper` 即启用——除了钱是假的,代码路径与实盘 100% 相同,用来暴露:行情时序问题、回调处理 bug、调度与重连逻辑、告警链路。

Paper 模式也必须写完整 `orders/trades/events` 表,并执行启动/盘中/收盘对账。区别只是对账对象从真实券商换成 `SimGateway` 的模拟账本。
