...
│   ├── contract/            #   __init__.py / types.py / strategy.py / context.py ← 03号文档
...
1. `strategies/` 只允许 import `core.contract` 与标准库/numpy/pandas——这是契约的物理保障。示例策略也必须使用 `from core.contract...`。
...
5. 策略不得读取 `runtime_mode`、环境变量或本地文件来判断当前运行环境。运行模式只属于启动器、引擎和报告。
...
| 层级 | 对象 | 要点 |
|---|---|---|
| 契约 | `core.contract` 数据类序列化、字段兼容、策略 import 白名单 | 先锁 API,再写引擎 |
| 数据语义 | `Bar.dt` 截止语义、动态复权基准日、缺失数据拒绝生产回测 | 防未来函数与静默脏数据 |
