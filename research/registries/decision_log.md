...
| 日期 | 决策 | 范围 | 证据 | 默认安全动作 |
| --- | --- | --- | --- | --- |
| 2026-07-02 | `CRYPTO_RESEARCH_TASKBOOK_LOCKED` | `crypto_trend_breadth_top2_v1` CIO 业务研究任务书 | `runs/2026-07-02T1030__crypto_trend_breadth_smoke_admission/08_cio_research_taskbook.md` 已预提交主候选、基线、消融实验、晋级规则和 kill/hold 标准；仍无审计 crypto 数据和正式 backtest | 保持 research-only；下一步只允许数据审查准备和按预提交矩阵生成正式证据；不得新增币种、调参追优、paper/live/交易所/真钱 |
| 2026-07-02 | `RESEARCH_ONLY_THESIS_DRAFT` | `a_share_limit_up_continuation_v0` A 股个股涨停后短线延续研究 | `runs/2026-07-02T1114__a_share_limit_up_continuation_v0` 已形成 brief、thesis、data requirements、experiment plan、CIO package；无审计数据、无正式回测、无风控审查；同日打板成交研究需要分钟/盘口/队列证据 | 保持 research-only；下一步只允许数据源选择和 data audit triage；不得 paper/live/QMT/券商/真钱 |
| 2026-07-02 | `SMOKE_ONLY_RESEARCH_CONTINUE` | `crypto_trend_breadth_top2_v1` synthetic smoke admission | `runs/2026-07-02T1030__crypto_trend_breadth_smoke_admission` 已生成 synthetic smoke 六件套；artifact inspector `PASS_WITH_WARNINGS`；Binance Spot probe `PROBE_PASS_WITH_WARNINGS_NOT_AUDITED_DATASET`；真实 crypto dataset 仍 `FAIL_FORMAL_DATASET`；正式 backtest 仍 `FAIL`；发现满仓目标扣费后现金小幅为负的风险 warning | 继续 research-only；下一步只允许正式 downloader、data audit、cross-source sanity check 和成本感知 sizing 研究；不得 paper/live/交易所/真钱 |
