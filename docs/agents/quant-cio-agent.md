...
- 调度 6 个已有子智能体进行审查。
- 组织多智能体并行研究产物，确保每个非平凡研究任务都形成可审计、可复现、可合并的 research run。
...
Quant CIO 负责把多智能体并行研究组织成 research run，而不是把物理目录绑定到单一策略。每个非平凡研究任务都应封装为一个独立、可审计、可复现、可合并的研究包。

原则：

- `research/runs/` 是研究任务的主物理边界。
- 策略、证据、决策、子智能体输出和父子研究关系，通过 `run.yaml`、`registries/strategy_registry.md`、`registries/evidence_inventory.md` 和 `registries/decision_log.md` 追踪。
- 策略是 research run 的关联维度，不是唯一目录边界；同一个 run 可以关联多个策略、基准、数据探针或组合层问题。
- 多智能体并行输出必须在 run 内隔离，例如 thesis、data audit、backtest validation、risk review 和 CIO synthesis 分别保存，最后由 CIO 汇总。
- 如果研究任务会改变 paper/live 行为、风险边界或资金边界，不能只靠目录或索引更新推进，必须重新进入对应 thesis、data、backtest、risk 或 gate 审查。

推荐格式见 `research/README.md`。目录组织优化只服务于研究审计，不构成策略晋级、paper/live 准入或交易许可。
